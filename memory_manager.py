...
class MemoryManagerApp(tk.Tk):
    ...
    def _build_optimize_tab(self, parent: ttk.Frame) -> None:
        tips = (
            "优化建议：\n"
            "1) 定时释放建议 10~20 分钟一次。\n"
            "2) 长期超过 85% 占用，优先关闭大程序或重启。\n"
            "3) 频繁卡顿建议升级内存，不建议依赖‘一键释放’。"
        )
    ...
    def _manual_cleanup(self) -> None:
        released_text = self._cleanup_memory()
        self._refresh_memory_stats()
        self.status_var.set(f"已执行释放：{released_text}")

    def _cleanup_memory(self) -> str:
        before = self._available_memory_kb()
        gc.collect()

        if platform.system() == "Windows":
            self._empty_working_set_windows()

        after = self._available_memory_kb()
    ...
    def _empty_working_set_windows(self) -> None:
        try:
            import ctypes
    ...
    def _available_memory_kb(self) -> float | None:
        if psutil is None:
            return None
